...
const SecondRemoteStream: React.FC<{ roomId: string }> = ({ roomId }) => {
  ...
  const channel = useRef<Camera2SignalServer | null>(null);

  useEffect(() => {
    // Initialize the signaling server
    if (!channel.current) {
      channel.current = new Camera2SignalServer(roomId);
    }
...
export default SecondRemoteStream;
